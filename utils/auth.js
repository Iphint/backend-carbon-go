...
export function signToken(user) {
  return jwt.sign(
    { id: user.id, username: user.username, email: user.email, role: user.role || "user", audience: "main" },
...
export function signAdminToken(user) {
  return jwt.sign(
    { id: user.id, username: user.username, email: user.email, role: user.role || "user", audience: "admin" },
...
export function setAdminAuthCookie(res, token) {
  res.cookie("carbon_go_admin_token", token, {
    httpOnly: true,
    secure: isProduction,
    sameSite: isProduction ? "none" : "lax",
    maxAge: 7 * 24 * 60 * 60 * 1000
...
export function clearAdminAuthCookie(res) {
  res.clearCookie("carbon_go_admin_token", {
    httpOnly: true,
    secure: isProduction,
    sameSite: isProduction ? "none" : "lax"
  });
